...
set LOG_PATH=%CONFIG_PATH%%LOG_PATH%\network
...
SET MAIN_CLASS=io.nuls.network.NetworkBootstrap
REM # JOPT_XMS reject
SET JOPT_XMS=256
rem JOPT_XMX reject
SET JOPT_XMX=512
rem # %JOPT_METASPACESIZE reject
SET JOPT_METASPACESIZE=128
rem # %JOPT_MAXMETASPACESIZE reject
SET JOPT_MAXMETASPACESIZE=256
rem  # %JAVA_OPTS reject
SET JAVA_OPT=--add-exports java.base/jdk.internal.misc=ALL-UNNAMED
set JAR_FILE=%MODULE_PATH%network-1.0.0.jar
...
SET JAVA_OPT=-server -XX:+UseG1GC -XX:MaxGCPauseMillis=50 -Xms256m -Xmx512m -XX:MetaspaceSize=128m -XX:MaxMetaspaceSize=256m -XX:+ParallelRefProcEnabled -XX:+TieredCompilation -XX:+ExplicitGCInvokesConcurrent
...
SET JAVA_OPT=%JAVA_OPT% %JAVA_GC_LOG% %JAVA_OOM_DUMP%  -Dapp.name=network


REM ECHO "account IS STARTING"
REM ECHO "account START CMD: %JRE_HOME%\bin\java %JAVA_OPT% %CLASSPATH% io.nuls.network.NetworkBootstrap %NulstarUrl% > %LOG_PATH%log.log"
REM ECHO "account log file : %LOG_PATH%log.log"

REM "%JRE_HOME%bin\java" %JAVA_OPT% %CLASSPATH% io.nuls.network.NetworkBootstrap %NulstarUrl%

"%JRE_HOME%\bin\java" %JAVA_OPT% %CLASSPATH% io.nuls.network.NetworkBootstrap %NulstarUrl%
