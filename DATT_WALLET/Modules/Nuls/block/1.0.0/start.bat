...
set LOG_PATH=%CONFIG_PATH%%LOG_PATH%\block
...
SET MAIN_CLASS=io.nuls.block.BlockBootstrap
REM # JOPT_XMS reject
SET JOPT_XMS=256
rem JOPT_XMX reject
SET JOPT_XMX=512
...
SET JAVA_OPT=
set JAR_FILE=%MODULE_PATH%block-1.0.0.jar
...
SET JAVA_OPT=%JAVA_OPT% %JAVA_GC_LOG% %JAVA_OOM_DUMP%  -Dapp.name=block


REM ECHO "account IS STARTING"
REM ECHO "account START CMD: %JRE_HOME%\bin\java %JAVA_OPT% %CLASSPATH% io.nuls.block.BlockBootstrap %NulstarUrl% > %LOG_PATH%log.log"
REM ECHO "account log file : %LOG_PATH%log.log"

REM "%JRE_HOME%bin\java" %JAVA_OPT% %CLASSPATH% io.nuls.block.BlockBootstrap %NulstarUrl%

"%JRE_HOME%\bin\java" %JAVA_OPT% %CLASSPATH% io.nuls.block.BlockBootstrap %NulstarUrl%
