...
set LOG_PATH=%CONFIG_PATH%%LOG_PATH%\ledger
...
SET MAIN_CLASS=io.nuls.ledger.LedgerBootstrap
REM # JOPT_XMS reject
SET JOPT_XMS=256
rem JOPT_XMX reject
SET JOPT_XMX=512
...
SET JAVA_OPT=
set JAR_FILE=%MODULE_PATH%ledger-1.0.0.jar
...
SET JAVA_OPT=%JAVA_OPT% %JAVA_GC_LOG% %JAVA_OOM_DUMP%  -Dapp.name=ledger


REM ECHO "account IS STARTING"
REM ECHO "account START CMD: %JRE_HOME%\bin\java %JAVA_OPT% %CLASSPATH% io.nuls.ledger.LedgerBootstrap %NulstarUrl% > %LOG_PATH%log.log"
REM ECHO "account log file : %LOG_PATH%log.log"

REM "%JRE_HOME%bin\java" %JAVA_OPT% %CLASSPATH% io.nuls.ledger.LedgerBootstrap %NulstarUrl%

"%JRE_HOME%\bin\java" %JAVA_OPT% %CLASSPATH% io.nuls.ledger.LedgerBootstrap %NulstarUrl%
