...
set LOG_PATH=%CONFIG_PATH%%LOG_PATH%\account
...
SET MAIN_CLASS=io.nuls.account.AccountBootstrap
REM # JOPT_XMS reject
SET JOPT_XMS=256
rem JOPT_XMX reject
SET JOPT_XMX=256
...
SET JAVA_OPT=%JAVA_OPT% %JAVA_GC_LOG% %JAVA_OOM_DUMP%  -Dapp.name=account


REM ECHO "account IS STARTING"
REM ECHO "account START CMD: %JRE_HOME%\bin\java %JAVA_OPT% %CLASSPATH% io.nuls.account.AccountBootstrap %NulstarUrl% > %LOG_PATH%log.log"
REM ECHO "account log file : %LOG_PATH%log.log"

REM "%JRE_HOME%bin\java" %JAVA_OPT% %CLASSPATH% io.nuls.account.AccountBootstrap %NulstarUrl%

"%JRE_HOME%\bin\java" %JAVA_OPT% %CLASSPATH% io.nuls.account.AccountBootstrap %NulstarUrl%
